...
/*
 ...
 Por isso deve-se ter o cuidado de não atribuir valores que podem ser nulos a variáveis que não são opcionais.
...
/*
 ...
 é possivel que na abertura ele não contenha nenhum valor e cause um erro em tempo de execução.
...
//ou faça dessa forma caso queira quebrar o texto antes do swift 4
var text = "This is some text\n"
         + "over multiple lines\n"
print(text)

//Use três aspas para quebrar uma grande string - disponível no swift 4
var text2 = """
    This is some text
    over multiple lines
    """
...
//exemplo com if ternário
var score = 88
var result = score > 85 ? "Passou" : "Falhou"
...
//FOR padrão
//Não use mais assim, use o outro modelo abaixo não se deve inclementar com ++x ou x++
...
print("O imc é :\(calcularIMC(peso: 114, altura: 1.87))") // Use assim ******************************************** Swift 3x
...
/**************************
24 - Map, Filter, Reduce, FlatMap
 ***************************/

//MAP
// O Map faz um loop em uma colação e aplica a mesma operação em cada elemento

//No exemplo abaixo imagine que temos um array com as notas de varios alunos e precisamos acrescentar 1 ponto extra para cada aluno, seria possível fazer essa operação com o comando For.
...
//A ideia central do map é fazer um loop sobre uma coleção, então vejamos como ficaria utilizando o map para fazer a mesma operação:

// Uma das váriações de sintaxes do map, utilizando closures (Um closure ocorre normalmente quando uma função é declarada dentro do corpo de outra, e a função interior referencia variáveis locais da função exterior.)

//SINTAXE 1
var alunosNotas2 = [4, 5, 7, 9, 6, 10, 3]
alunosNotas2 = alunosNotas2.map({(nota:Int) -> Int in
    return nota + 1
})
print(alunosNotas2)
...
//SINTAXE 2
var alunosNotas3 = [4, 5, 7, 9, 6, 10, 3]
alunosNotas3 = alunosNotas3.map{(nota:Int) in
    return nota + 1
}
print(alunosNotas3)
...
//SINTAXE 3
var alunosNotas4 = [4, 5, 7, 9, 6, 10, 3]
alunosNotas4 = alunosNotas4.map{nota in nota + 1}
print(alunosNotas4)



//Além das formas mostradas acima, podemos utilizar o map com a syntax sugar com $, onde $0 é o elemento atual, $1 próximo elemento e assim sucessivamente:

//SINTAXE 4
var alunosNotas5 = [4, 5, 7, 9, 6, 10, 3]
alunosNotas5 = alunosNotas5.map {$0 + 1}
print(alunosNotas5 )

//Resumindo são todas iguais apenas é uma questão de preferência de sintaxe longa ou abreviada, veja todos os exemplos em uma só linha:
...
//Faz um loop sobre uma coleção e retorna uma coleção que contém elementos que atendem a uma condição. Digamos que temos o nosso mesmo array com as notas dos alunos e queremos filtar apenas os alunos aprovados. Para ser aprovado é necessario que o aluno tenha uma nota 6.0 ou maior. Poderiamos facilmente fazer um for e comparar cada elemento:


var alunosNotas6 = [4, 5, 7, 9, 6, 10, 3]
var aprovadosNotas:[Int] = []

/* Percorre todo array de alunosNotas6, e filtra quem tem nota maio igual a 6
 */
for nota in alunosNotas6 {
    if nota >= 6 {
...
//Com isso teriamos no array aprovadosNotas as notas dos alunos aprovados. A ideia central do filter é essa, fazer um loop sobre uma coleção e aplicar un “filtro”, então vejamos como ficaria utilizando o filter para fazer a mesma operação. Podemos utilizar o filter com a sintaxe de $ vista anteriormente:


// Verifica se a nota é >= a 6
...
//Reduce
//Combina todos os itens de uma coleção para criar um único valor. Vamos supor que precisamos obter a soma de todas as notas dos alunos, uma solução seria implementar outro loop For-In:
...
//No último exemplo, o operador + é substituído pelo fechamento {$ 0 + $ 1}, criando uma sintaxe ainda mais simplificada e acessível.Curiosamente, a mesma regra se aplica a matrizes de string também:

var testeString = ["um", "dois", "três", "quatro"].reduce("", {$0 + $1})
print(testeString)

//FlatMap nivela uma coleção de coleções
//Digamos que temos dois arrays dentro de um array que gostaríamos de combinar em um único array. Como faríamos isso? Provavelmente com outro loop For-In:

let arrayNoArray = [[11,12,13],[14,15,16]]

var novoArray: [Int] = []
for array in arrayNoArray{
    novoArray += array
}
print(novoArray)
...
print(pessoasValidas)

//Encadeamento
//É quando as funções de ordem superior realmente brilham. Você pode combinar todas as funções que acabou de aprender em uma linha de código, digamos que queremos o quadrados de todos os números pares de uma matriz de matrizes. Em vez de escrever um monte de loops for e ifs, podemos usar as funções de ordem superior para simplificar nosso código e obter os mesmos resultados.



let arrayDosQuadradosNumerosPares = arrayNoArray.flatMap{$0}.filter{$0 % 2 == 0}.map{$0 * $0}
print(arrayDosQuadradosNumerosPares)

//Outro exemplo de encadeamento que vamos mostrar são de inteiros que representam dinheiro inserindo um $ na frente de cada elemento no exemplo arrayNoArray usando flatMap e map.

var arrayComCifrao = arrayNoArray.flatMap{$0}.map{"$\($0)"}
print(arrayComCifrao)
